...
def train_val_split(
    ...
    """
    Crée une split train/val/test simple (avec stratification optionnelle).

    Args:
        cases: Liste des noms de patients
        val_size: Proportion pour validation (défaut: 0.2)
        test_ratio: Proportion pour test (défaut: 0.0)
    ...
    if stratified:
        ...
        for c in range(num_classes):
            ...
            split_idx1 = max(1, int(len(class_cases) * (1 - val_size - test_ratio)))
            split_idx2 = max(split_idx1 + 1, int(len(class_cases) * (1 - test_ratio)))

            train_cases.extend(shuffled[:split_idx1])
            val_cases.extend(shuffled[split_idx1:split_idx2])
            test_cases.extend(shuffled[split_idx2:])
        ...
        split_idx1 = int(len(cases) * (1 - val_size - test_ratio))
        split_idx2 = int(len(cases) * (1 - test_ratio))

        train_cases = shuffled[:split_idx1]
        val_cases = shuffled[split_idx1:split_idx2]
        test_cases = shuffled[split_idx2:]
    ...
    test_df = create_df(test_cases, preprocessed_dir)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Proportion pour test (défaut: 0.0)"
    )
    parser.add_argument(
        "--kfold",
        type=int,
    ...
    if args.kfold:
        ...
        print(f"📊 Création d'une split train/val/test ({1-args.test_size-args.test_ratio:.0%}/{args.test_size:.0%}/{args.test_ratio:.0%})...")

        train_df, val_df, test_df = train_val_split(
            cases,
