...
```bash
# Pipeline complète avec split train/val stratifié (recommandé)
python pipeline.py \
...
# Cross-validation 5-fold avec stratification
python pipeline.py \
    --raw_data_dir /path/to/raw_data \
    --architectures SegFormer3D \
    --preprocessed_data_dir ./preprocessed_data \
...
python pipeline.py --target_size 128 --architectures SegFormer3D
```
...
- **Automatisation complète** : Pipeline zero-touch pour l'expérimentation</content>
<parameter name="filePath">d:\data\documents\ING3_2025-2026\Projet_PIC\pic\SegFormer3D\README_PIPELINE.md