...
class ProstateSegDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            target_size: Taille cible pour le redimensionnement.
                Peut être un int (96 -> 96x96x96) ou une liste [X, Y, Z] pour anisotropique.
                Defaults to 96.
        ...
        Exemple:
            >>> # Isotropique
            >>> dataset = ProstateSegDataset(root_dir="./data", target_size=96)
            >>> # Anisotropique
            >>> dataset = ProstateSegDataset(root_dir="./data", target_size=[64, 64, 32])
        """
        super().__init__()
        ...
        if isinstance(target_size, (list, tuple)):
            ...
            size_str = f"{target_size[0]}x{target_size[1]}x{target_size[2]}"
        else:
            self.target_size = (target_size, target_size, target_size)
            size_str = f"{target_size}³"
        
        print(f" Chargé {len(self.csv)} patients depuis {csv_fp} (cible: {size_str})")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            # Détermine la taille cible (tuple de 3 valeurs)
            ...
            current_shape = volume.shape[-3:]  # (D, H, W)
